...
import requests
import os 
key = os.environ.get("STRAIN_KEY")

# this file needs to reside inside a management folder in the django app

def effect_data():
    # Importing effects 
    res = requests.get("http://strainapi.evanbusse.com/{}/searchdata/effects".format(key))
...
def flavours_data():
    ...
    res = requests.get("http://strainapi.evanbusse.com/{}/searchdata/flavors".format(key))
    body = json.loads(res.content)
    flavours_seed = body
